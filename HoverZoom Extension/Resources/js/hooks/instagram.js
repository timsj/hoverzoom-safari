...
(function () {
  ...
  function notify(url, text) {
    try {
      ...
      if (
        text.indexOf("image_versions2") === -1 &&
        text.indexOf("display_url") === -1 &&
        text.indexOf("carousel_media") === -1 &&
        text.indexOf("video_versions") === -1 &&
        text.indexOf('"tray"') === -1 &&
        text.indexOf("reels_media") === -1 &&
        text.indexOf("profile_pic_url") === -1
      ) {
        return;
      }
      document.dispatchEvent(
        new CustomEvent("hzInstagramPayload", {
  ...
  document.addEventListener("hzInstagramFetchRequest", function (e) {
    try {
      ...
      if (!req) return;
      if (req.mediaId) {
        fetchIG("/api/v1/media/" + req.mediaId + "/info/")
          .then(function () {
            if (req.shortcode)
              return fetchIG("/p/" + req.shortcode + "/?__a=1&__d=dis");
          })
          .then(function () {
            if (req.shortcode)
      ...
      if (req.username) {
        fetchIG(
          "/api/v1/users/web_profile_info/?username=" +
            encodeURIComponent(req.username),
        );
      }
    } catch (e) {}
  });
  ...
  document.addEventListener("hzInstagramHighlightRequest", function (e) {
    try {
      ...
      if (req.highlightId) {
        fetchIG(
          "/api/v1/feed/reels_media/?reel_ids=highlight:" +
            encodeURIComponent(req.highlightId),
        );
      } else if (req.userId) {
        fetchIG(
          "/api/v1/highlights/" +
