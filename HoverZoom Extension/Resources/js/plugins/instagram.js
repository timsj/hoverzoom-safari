...
hoverZoomPlugins.push({
  ...
  prepareImgLinks: function (callback) {
    ...
    function add(el) {
      ...
      var d = el.data();
      if (!(
        (d.hoverZoomSrc && d.hoverZoomSrc.length) ||
        (d.hoverZoomGallerySrc && d.hoverZoomGallerySrc.length)
      ))
        return;
    ...
    function igRequestHeaders() {
      ...
      var csrf =
        typeof hoverZoom.getCookie === "function"
          ? hoverZoom.getCookie("csrftoken")
          : null;
      if (csrf) {
    ...
    function askPageBridge(eventName, detail) {
      document.dispatchEvent(
        new CustomEvent(eventName, { detail: JSON.stringify(detail) }),
      );
    }
    ...
    function requestUserProfile(username, onProfile) {
      ...
      if (userFailures[username] && Date.now() - userFailures[username] < 60000)
        return;
        ...
        function (data) {
          ...
          if (apiUser) {
            usersCache[username] = {
              ...
              profile_pic_url:
                apiUser.profile_pic_url_hd || apiUser.profile_pic_url || "",
            };
            saveUsersCache();
          } else {
    ...
    function requestPostData(shortcode) {
      ...
      if (
        state === true ||
        (typeof state === "number" && Date.now() - state < 60000)
      )
        return;
      ...
      askPageBridge("hzInstagramFetchRequest", {
        shortcode: shortcode,
        mediaId: mediaId,
      });
      igGet(
    ...
    function storyUserId(username) {
      ...
      if (usersCache[username] && usersCache[username].id)
        return usersCache[username].id;
    ...
    function applyStoryResponse(data, wantedKey) {
      if (
        !data ||
        !Array.isArray(data.reels_media) ||
        data.reels_media.length === 0
      )
        return false;
      var fetched = {};
      ...
      requestOnce(
        ...
        function (finish) {
          ...
          var askPage = function (id) {
            cLog(
              "instagram: asking the page for the story of " +
                username +
                (id ? " (id " + id + ")" : ""),
            );
            ...
            igGetText(
              ...
              function (html) {
                finish(
                  applyStoryFromHtml(
                    html,
                    "story_" + String(username).toLowerCase(),
                  ),
                );
              },
            );
            ...
            igGet(
              ...
              function (data) {
                ...
                cLog(
                  "instagram: story of " +
                    username +
                    " via reels_media failed, trying the story page",
                );
                tryPage(id);
              },
            );
    ...
    function requestHighlightsTray(username, onDone) {
      ...
      var fetchTray = function (uid) {
        ...
        igGet(
          "https://www.instagram.com/api/v1/highlights/" +
    ...
    function requestHighlightData(highlightId, onUnavailable) {
      ...
      requestOnce(storyRequests, key, onUnavailable, function (finish) {
        askPageBridge("hzInstagramHighlightRequest", {
    ...
    function refreshMediaElements() {
      $(".hoverZoomLink").each(function () {
        ...
        var postData =
          getPostFromMap(mediaMap, sc) ||
          (storyKey ? getStoryFromMap(mediaMap, storyKey) : null);
        if (!postData) return;

        var applied = sc
          ? applyPostMediaIfNew(el, sc, postData)
          : storyKey
            ? applyMediaIfNew(
                el,
                "hoverZoomAppliedStoryKey",
                storyKey,
                postData,
              )
            : false;
    ...
    function processPayload(payload) {
      ...
      cLog(
        "instagram: page bridge delivered " +
          keys.length +
          " entries (" +
          String(payload.url).slice(0, 90) +
          ")",
      );
      mergeMediaData(fetched);
    }
    ...
    function processUserPayload(data) {
      ...
      cLog(
        "instagram: profile of " + apiUser.username + " via the page bridge",
      );
    ...
    function getBestImageUrl(imgObj, displayUrl, displayResources) {
      if (
        imgObj &&
        Array.isArray(imgObj.candidates) &&
        imgObj.candidates.length
      ) {
        var sorted = imgObj.candidates.slice().sort(function (a, b) {
          return (
            b.width * (b.height || b.width) - a.width * (a.height || a.width)
          );
        });
        return sorted[0].url;
      }
      if (Array.isArray(displayResources) && displayResources.length) {
        var sortedRes = displayResources.slice().sort(function (a, b) {
          return (
            b.config_width * (b.config_height || b.config_width) -
            a.config_width * (a.config_height || a.config_width)
          );
        });
    ...
    function mediaFromItem(item, caption, dashManifest) {
      ...
      if (
        videoUrl &&
        (item.is_video || item.media_type === 2 || item.video_versions)
      ) {
        return { type: "video", url: videoUrl + ".video", caption: caption };
      }
      var imgUrl = getBestImageUrl(
    ...
    function parseMediaNode(node) {
      ...
      if (
        ...
        carouselItems = node.edge_sidecar_to_children.edges.map(function (e) {
          return e.node;
        });
      }
      ...
      if (Array.isArray(carouselItems) && carouselItems.length > 0) {
        ...
        for (var i = 0; i < carouselItems.length; i++) {
          ...
          var media = mediaFromItem(
            item,
            caption,
            item.video_dash_manifest || item.dash_manifest,
          );
      ...
      return mediaFromItem(
        node,
        caption,
        node.video_dash_manifest || node.dash_manifest,
      );
    }
    ...
    function processStoriesTray(data, map) {
      ...
      var tray =
        data.tray || data.reels_media || (data.data && data.data.reels_media);
      ...
      for (var i = 0; i < tray.length; i++) {
        ...
        var isHighlight =
          /^highlight:/.test(reelId) || reel.reel_type === "highlight";
    ...
    function processMediaObject(node, map) {
      ...
      if (
        ...
        /^[a-zA-Z0-9._]{1,30}$/.test(node.username)
      ) {
        var rawUserId = node.pk || node.id || node.user_id || node.owner_id;
        if (
          rawUserId !== undefined &&
          rawUserId !== null &&
          /^\d+$/.test(String(rawUserId))
        ) {
    ...
    function setGallery(el, gallery, captions, fallbackCaption) {
      ...
      el.data().hoverZoomCaption =
        (captions && captions[idx]) || fallbackCaption || "";
    }
    ...
    function applyMediaIfNew(el, marker, value, postData) {
      ...
      var newUrl = postData.url
        ? ensureHzUrl(postData.url)
        : postData.gallery &&
          postData.gallery[0] &&
          ensureHzUrl(postData.gallery[0][0]);
    ...
    function applyPostMediaIfNew(el, shortcode, postData) {
      return shortcode
        ? applyMediaIfNew(el, "hoverZoomAppliedShortcode", shortcode, postData)
        : false;
    }
    ...
    function isAvatarImg(img) {
      ...
      if (
        /profile picture|profile photo/i.test(alt) ||
        /['’]s\s+(?:story|stories|profile picture)/i.test(alt)
      )
        return true;
      ...
      if (
        img.closest(
          'header, [role="button"]:has(canvas), [role="link"]:has(canvas)',
        ).length > 0
      )
        return true;
      return false;
    ...
    var mediaJsonMarker =
      /image_versions2|display_url|display_resources|video_versions|video_url|carousel_media|edge_sidecar|reels_media|"tray"|cover_media|reel_type|"username"|"pk"/;
    ...
    if (
      !window.__hzInstagramBridgeInjected &&
      ...
      hookScript.src = browser.runtime.getURL("js/hooks/instagram.js");
      ...
      el.on("mouseenter mouseover", function () {
        ...
        if (sc) {
          ...
          if (
            postData &&
            (postData.type === "video" ||
              (!nativeVideo && (postData.url || postData.gallery)))
          ) {
    ...
    $("article").each(function () {
      ...
      var shortcode = null;
      article
        .find('a[href*="/p/"], a[href*="/reel/"], a[href*="/reels/"]')
        .each(function () {
      ...
      if (!shortcode) {
        ...
        if (timeLink.length) {
          var m = (timeLink.attr("href") || "").match(
            /\/(p|reel|reels)\/([^/?#]+)/,
          );
      ...
      var avatarImg = article
        ...
        .first();
      if (avatarImg.length) {
        var avTarget = avatarImg.closest(
          'div[role="button"], span[role="link"], a',
        );
      ...
      if (video.length) {
        ...
        if (!vTarget.length)
          vTarget = video.closest(
            'div[role="button"], div:has(> div > video), div:has(> video)',
          );
        if (!vTarget.length) vTarget = video.parent();
        var posterImg = article
          .find(
            'img[src*="cdninstagram.com"], img[src*="fbcdn.net"], img[srcset]',
          )
          .filter(function () {
            return (
              !isAvatarImg($(this)) && $(this).closest("header").length === 0
            );
          })
          .first();
        ...
        var directVideoUrl =
          vEl.currentSrc &&
          /^https?:/.test(vEl.currentSrc) &&
          !/^blob:/.test(vEl.currentSrc)
            ? vEl.currentSrc + ".video"
            : null;
        ...
        images.each(function () {
          ...
          if (!target.length)
            target = img.closest(
              'div[role="button"], div:has(> div > img), div:has(> img)',
            );
          ...
          if (postData) {
            ...
            var captions = [];
            article
              .find('ul li img, div[role="presentation"] img')
              .filter(function () {
                return !isAvatarImg($(this));
              })
              .each(function () {
                var sBest =
                  getBestFromSrcset($(this).attr("srcset")) || this.src;
                if (
                  sBest &&
                  !slides.some(function (s) {
                    return s[0] === ensureHzUrl(sBest);
                  })
                ) {
    ...
    $('a[href*="/p/"], a[href*="/reel/"], a[href*="/reels/"]').each(
      function () {
        var link = $(this);
        ...
        if (postData) {
          ...
          if (vUrl) {
            ...
            var img = link
              .find(
                'img[src*="cdninstagram"], img[src*="fbcdn"], img[srcset], img[src]',
              )
              .first();
            ...
            if (img.length) {
              placeholderFromImg(
                link,
                img,
                img.attr("alt") || link.attr("aria-label") || "",
              );
            }
          }
        }
    ...
    function isStoryRing(el) {
      ...
      return (
        el.closest(
          'a[href*="/stories/"], [role="button"]:has(canvas), [role="link"]:has(canvas)',
        ).length > 0
      );
    }
    ...
    function extractStoryUsername(target) {
      ...
      if (!storyLink.length)
        storyLink = target.find('a[href*="/stories/"]').first();
      if (storyLink.length) {
    ...
    function extractHighlightId(target) {
      if (!target || !target.length) return null;
      var link = target.is('a[href*="/stories/highlights/"]')
        ? target
        : target.closest('a[href*="/stories/highlights/"]');
      if (!link.length)
        link = target.find('a[href*="/stories/highlights/"]').first();
    ...
    function looksLikeOwnerStory(target) {
      ...
      if (
        img.length &&
        /profile picture|profile photo|story/i.test(img.attr("alt") || "")
      )
        return true;
      return false;
    }
    ...
    function getStoryData(target) {
      var highlightId =
        extractHighlightId(target) || highlightIdFromCover(target);
    ...
    function isLikelyCover(target) {
      if (
        target.closest('[role="menu"], [role="menuitem"], nav, footer').length
      )
        return false;
      var img = target.is("img") ? target : target.find("img").first();
    ...
    function prepareStoryTargets() {
      $(storySelector).each(function () {
        ...
        if (
          el.closest(
            'article, a[href*="/p/"], a[href*="/reel/"], a[href*="/reels/"]',
          ).length
        )
          return;
        var target = el.closest(
          'a[href*="/stories/"], div[role="button"], button',
        );
        ...
        var isExplicitStory =
          target.is('a[href*="/stories/"]') ||
          target.closest('a[href*="/stories/"]').length > 0;
        if (!story.storyKey && !story.postData && !isExplicitStory) {
          ...
          if (isLikelyCover(target)) {
            var coverImg = target.is("img")
              ? target
              : target.find("img").first();
            ...
            .on("mouseenter mouseover", function () {
              ...
              cLog(
                "instagram: story hover " +
                  (s.highlightId
                    ? "highlight:" + s.highlightId
                    : s.username || "(unresolved)") +
                  (s.postData ? " (cached)" : ""),
              );
              ...
              if (s.postData) {
                ...
                placeholderFromImg(
                  link,
                  link.is("img") ? link : link.find("img").first(),
                );
                askPageBridge("hzInstagramStoryRequest", {});
              }
            })
        ...
        if (story.postData) {
          ...
          placeholderFromImg(
            target,
            target.is("img") ? target : target.find("img").first(),
          );
        }
    ...
    $("a[href]")
      .filter(function () {
        return !/(\/reel\/|\/p\/|\/explore\/|\/stories\/)/.test(
          $(this).prop("href"),
        );
      })
      .each(function () {
        ...
        if (link.data().hoverZoomStoryBound || link.data().hoverZoomStoryKey)
          return;
