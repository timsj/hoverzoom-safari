...
hoverZoomPlugins.push({
  ...
  prepareImgLinks: function () {
    ...
    function apiMedia(url, pick) {
      if (!self.api.has(url)) {
        if (self.api.size >= cacheLimit)
          self.api.delete(self.api.keys().next().value);
        self.api.set(
          url,
          fetch(url, {
            headers: { "x-ig-app-id": webAppId },
            credentials: "include",
          })
    ...
    function postMedia(shortcode) {
      ...
      if (String(id).length <= 20)
        return apiJson(`/api/v1/media/${id}/info/`, (data) =>
    ...
    function reelMedia(id) {
      return apiJson(
        `/api/v1/feed/reels_media/?reel_ids=${id}`,
    ...
    function profileStories() {
      ...
      const shortcode =
        link && (link.getAttribute("href").match(shortcodeRe) || [])[1];
      return shortcode
        ? postMedia(shortcode).then((post) =>
            post && post.user ? reelMedia(post.user.pk) : null,
          )
        : null;
    }
    ...
    function trayStories(names) {
      return apiJson(
        ...
        const reel = (tray || []).find(
          (reel) => reel.user && names.includes(reel.user.username),
        );
    ...
    function mediaSrc(media) {
      return media.video_versions
        ? [media.video_versions[0].url + ".video"]
    ...
    function onHover(event) {
      ...
      if (
        !hit.closest ||
        hit.closest("#hzViewer") ||
        hit.closest(".hoverZoomLink")
      )
        return; // our viewer, or already zoomable
    ...
    function showStories(circle, face) {
      ...
      const stories =
        (names.includes(viewed) && profileStories()) || trayStories(names);
      const retry = () => self.resolved.delete(circle); // nothing to show: allow another hover
      stories
        .then((items) =>
          items
            ? hoverZoom.prepareLink($(circle), items.map(mediaSrc))
            : retry(),
        )
        .catch(retry);
    }
    ...
    function showPostMedia(hit) {
      ...
      const post =
        highlight ||
        hit.closest("article") ||
        hit.closest('a[href*="/p/"], a[href*="/reel/"]');
      if (!post || self.resolved.has(post)) return;
      const media = post.querySelector(
        'img[src*="cdninstagram"]:not([src*="-19/"]), video',
      );
      ...
      const zoom = (srcs) => {
        ...
        if (frame.matches(":hover"))
          pauseBehindPreview(post.querySelector("video"), post);
      };
      const fallback = () => {
        // the page's own full size photo
        const cover = post.querySelector(
          'img[src*="cdninstagram"]:not([src*="-19/"])',
        );
      ...
      if (highlight) {
        const id = (highlight.getAttribute("href").match(/highlights\/(\d+)/) ||
          [])[1];
        if (id)
          reelMedia(`highlight:${id}`)
            .then((items) => show(items && items.map(mediaSrc)))
            .catch(fallback);
        else fallback();
      } else {
        const link = post.matches("a[href]")
          ? post
          : post.querySelector('a[href*="/p/"], a[href*="/reel/"]');
        const shortcode =
          link && (link.getAttribute("href").match(shortcodeRe) || [])[1];
          ...
          postMedia(shortcode)
            .then((item) =>
              show(
                item &&
                  (item.media_type === 8
                    ? item.carousel_media.map(mediaSrc)
                    : mediaSrc(item)[0]),
              ),
            )
            .catch(fallback);
    ...
    function pauseBehindPreview(video, container) {
      if (
        !video ||
        video.paused ||
        self.pausedVideos.some((entry) => entry[0] === video)
      )
        return;
