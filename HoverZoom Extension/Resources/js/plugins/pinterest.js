...
hoverZoomPlugins.push({
  ...
  prepareImgLinks: function (callback) {
    ...
    patches.forEach((patch) => {
      hoverZoom.urlReplace(res, "img[src]", /\/\d+x\d+_RS\//, patch);
    });

    // pin images (with or without srcset)
    patches.forEach((patch) => {
      hoverZoom.urlReplace(res, "img[src]", /\/\d+x(\d+)?\//, patch, [
        'a[href*="/pin/"]',
    ...
    $("img[srcset]").each(function () {
      ...
      let biggestSrc = hoverZoom.getBiggestSrcFromSrcset(
    ...
    $('a[href*="/pin/"]').on("mouseenter", function () {
      ...
      if (link.data().hoverZoomPinVideoUrl) {
        ...
        if (link.data().hoverZoomPinCaption)
      ...
      let pinData = null;
      const docScript =
        document.getElementById("__PWS_INITIAL_PROPS__") ||
      ...
      function processPinData(data) {
        ...
        const caption =
          data.rich_metadata?.title || data.title || data.seo_title;
      ...
      const hasVideoIndicator =
        link.find(
          'video, [data-test-id*="video"], [aria-label*="video"], [aria-label*="Video"]',
        ).length > 0 ||
        link
          .closest('div[data-test-id="pinWrapper"]')
          .find(
            'video, [data-test-id*="video"], [aria-label*="video"], [aria-label*="Video"]',
          ).length > 0;
      ...
      if (
        !hasVideoIndicator &&
        link.data().hoverZoomSrc &&
        link.data().hoverZoomSrc.length > 0
      ) {
        link.data().hoverZoomPinLoading = false;
        link.data().hoverZoomPin = pin;
        return;
        ...
        function (response) {
          ...
          try {
            ...
            const script = Array.from(doc.scripts).find(
              (s) =>
                s.id === "__PWS_INITIAL_PROPS__" || s.id === "__PWS_DATA__",
            );
            if (!script) {
              link.data().hoverZoomPinLoading = false;
              return;
