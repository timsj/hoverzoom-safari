...
function Logger() {
  ...
  if (debug) {
    ...
    this.logger.enterFunc = function () {
      console.info("enter: " + this.enterFunc.caller.name);
    };
    this.logger.leaveFunc = function () {
      console.info("leave: " + this.leaveFunc.caller.name);
    };
  } else {
    for (var m in console) {
      if (typeof console[m] == "function") this.logger[m] = function () {};
    }
...
var hoverZoom = {
  ...
  loadHoverZoom: function () {
    ...
    function posViewer(position) {
      ...
      var offset = 20,
        ...
        scrollBarHeight =
          !options.hScrollBarOverlap && hasScrollbarH() ? 17 / zoom : 0,
        ...
        docStyleLeft = document.documentElement
          ? Math.abs(parseInt(document.documentElement.style.left) || 0)
          : 0,
        docStyleTop = document.documentElement
          ? Math.abs(parseInt(document.documentElement.style.top) || 0)
          : 0,
        docScrollLeft = document.documentElement
          ? document.documentElement.scrollLeft
          : 0,
        docScrollTop = document.documentElement
          ? document.documentElement.scrollTop
          : 0,
        wndScrollLeft = Math.max(
          bodyStyleLeft,
          bodyScrollLeft,
          docStyleLeft,
          docScrollLeft,
        ),
        wndScrollTop = Math.max(
          bodyStyleTop,
          bodyScrollTop,
          docStyleTop,
          docScrollTop,
      ...
      function adjustCaptionMiscellaneousDetails() {
        ...
        if (hzCaptionMiscellaneous) hzCaptionMiscellaneous.css("max-width", 0);
        if (hzDetails) hzDetails.css("max-width", 0);
        if (hzAbove) hzAbove.css("max-width", 0);
        if (hzBelow) hzBelow.css("max-width", 0);
        ...
        const hzAboveHeight =
          (fullZoomKey && hideDetailsandCaptions) || !hzAbove
            ? padding
            : hzAbove.height();
        const hzBelowHeight =
          (fullZoomKey && hideDetailsandCaptions) || !hzBelow
            ? padding
            : hzBelow.height();
        ...
        while (
          !viewerLocked &&
          hz.hzViewer.height() >
            wndHeight - statusBarHeight - scrollBarHeight &&
          i++ < 10
        ) {
          imgFullSize
            .height(
              wndHeight -
                ...
                statusBarHeight -
                scrollBarHeight -
                (hzAbove ? hzAboveHeight : 0) -
                (hzBelow ? hzBelowHeight : 0),
            )
            .width("auto");
        }
        ...
        if (
          imgFullSize[0].clientWidth < 50 ||
          (fullZoomKey && hideDetailsandCaptions)
        ) {
          ...
          if (hzDetails) {
            ...
            let i = 0;
            while (
              hzDetails[0].scrollWidth - 1 <= hzDetails[0].clientWidth &&
              i < nb
            ) {
              let detail = hzDetails.find(".hzDetail").eq(i++);
      ...
      if ($(hzAbove).height() == 0)
        hz.hzViewer.css({ "padding-top": `${padding}px` });
      if ($(hzBelow).height() == 0)
        hz.hzViewer.css({ "padding-bottom": `${padding}px` });
      ...
      if ($("#hzLoader.imgLoading")[0] != undefined) {
        ...
        if (viewerLocked) {
          ...
          imgFullSize.width(
            Math.min(
              srcDetails.naturalWidth * zoomFactor,
              wndWidth - offset - padding - 2 * scrollBarWidth,
            ),
          );
        } else if (displayOnRight) {
          if (
            srcDetails.naturalWidth * zoomFactor + padding >
            wndWidth - position.left
          ) {
            imgFullSize.width(
              wndWidth - position.left - padding + wndScrollLeft,
            );
          }
        } else {
        ...
        if (
          !viewerLocked &&
          hz.hzViewer.height() >
            wndHeight - padding - statusBarHeight - scrollBarHeight
        ) {
          imgFullSize
            .height(wndHeight - padding - statusBarHeight - scrollBarHeight)
            .width("auto");
        }
      ...
      if (options.centerImages || viewerLocked) {
        ...
        if (
          position.left + imgFullSize.width() + 2 * padding >
          wndScrollLeft + wndWidth
        ) {
          position.left = wndScrollLeft + 0.5 * padding;
        }
    ...
    const videoExtensionsWithGif = new Set([
      "3gpp",
      "gif",
      "gifv",
      "m4v",
    ...
    function isVideoLink(url, includeGifs = false) {
      ...
      return (
        (includeGifs
          ? videoExtensionsWithGif.has(ext)
    ...
    function getVideoAudioSubtitlesFromUrl() {
      ...
      const subtitlesurl =
        audiosubtitlesurl.split(".audiomuted")[1] ||
        audiosubtitlesurl.split(".audio")[1] ||
        audiosubtitlesurl;
      const audiourl =
        audiosubtitlesurl.indexOf(".audio") != -1
          ? audiosubtitlesurl.indexOf(".audiomuted") != -1
            ? audiosubtitlesurl.split(".audiomuted")[0] + ".audiomuted"
            : audiosubtitlesurl.split(".audio")[0] + ".audio" || ""
          : "";
      ...
      if (subtitlesurl.endsWith(".subtitles")) {
        srcDetails.subtitlesUrl = subtitlesurl
          .replace(/^_/, "")
          .replace(".subtitles", "");
      }
    ...
    function updateAmbilight() {
      ...
      if (
        !(
          isVideo ||
          (imgFullSize.get(0).complete && imgFullSize.get(0).naturalWidth)
        )
      ) {
      ...
      if (height > width) {
        scaleH = scale;
        scaleW = (width + height * (scale - 1)) / width;
      }
      if (height < width) {
        scaleW = scale;
        scaleH = (height + width * (scale - 1)) / height;
      }
      ...
      $(canvas)
        ...
        .css(
          "transform",
          " scale(" + transfoX * scaleW + "," + transfoY * scaleH + ") ",
        );
    ...
    function posWhileLoading() {
      if (loading) {
        posViewer();
        if (
          $("#hzLoader.imgLoading")[0] != undefined &&
          imgFullSize &&
          imgFullSize.height() > 0
        ) {
    ...
    function removeTitles(img) {
      ...
      titledElements = img
        .parents("[title]")
        .not('iframe, .lightbox, [rel^="lightbox"]');
    ...
    function frameBackgroundColor(color) {
      imgFullSizeCss.borderColor =
        ...
        hzDetailCss.backgroundColor =
        hzMiscellaneousCss.backgroundColor =
          color;

      if (options.fontOutline) {
        // outline text: white font + thin black border
        hzCaptionCss.color =
          hzMiscellaneousCss.color =
          hzDetailCss.color =
            "white";
        hzCaptionCss.textShadow =
          hzMiscellaneousCss.textShadow =
          hzDetailCss.textShadow =
    ...
    function fontSize(size) {
      size = parseInt(size);
      hzCaptionCss.fontSize =
        hzMiscellaneousCss.fontSize =
        hzDetailCss.fontSize =
          size + "px";
      ...
      hzCaptionCss.paddingLeft =
        hzMiscellaneousCss.paddingLeft =
        hzCaptionCss.paddingRight =
        hzMiscellaneousCss.paddingRight =
          0.5 * size + "px";
      hzCaptionCss.marginLeft =
        hzMiscellaneousCss.marginLeft =
        hzDetailCss.marginLeft =
          size + "px";
      hzCaptionCss.marginRight = hzMiscellaneousCss.marginRight = size + "px";
      hzCaptionCss.borderRadius =
        hzMiscellaneousCss.borderRadius =
        hzDetailCss.borderRadius =
          0.25 * size + "px";
    ...
    function documentMouseMove(event) {
      ...
      if (
        ...
        if (
          mousePos.top > linkRect.top &&
          mousePos.top < linkRect.bottom &&
          mousePos.left > linkRect.left &&
          mousePos.left < linkRect.right
        ) {
          return;
        }
      }
      ...
      if (links && links.length > 0) {
        ...
        if (
          ...
          if (!imgFullSize) {
            ...
            if (
              links.data().hoverZoomSrc &&
              (!options.actionKey || actionKeyDown)
            ) {
              const audioSrc = links.data().hoverZoomAudioSrc
                ? hoverZoom.getFullUrl(
                    links.data().hoverZoomAudioSrc[hoverZoomSrcIndex],
                  )
                : undefined;
              ...
              const delay =
                actionKeyDown || explicitCall
                  ? 0
                  : isVideoLink(srcDetails.url)
    ...
    function mouseAction(mouseButtonKey, img, event) {
      preventDefaultMouseAction(
        imgFullSize || !hideKeyDown ? true : false,
        mouseButtonKey,
      );
    ...
    function mouseButtonKeyHandler(mouseButtonKey, img, event) {
      ...
      switch (mouseButtonKey) {
        case -1:
        case -3:
          longRightPressTimer = setTimeout(
            longClick.bind(img),
            timerDuration,
            mouseButtonKey,
            event,
        ...
        case -2:
        case -4:
          longMiddlePressTimer = setTimeout(
            longClick.bind(img),
            timerDuration,
            mouseButtonKey,
            event,
    ...
    function documentMouseDown(event) {
      ...
      if (
        event.button === 0 &&
        imgFullSize &&
        event.target !== hz.hzViewer[0] &&
        event.target !== imgFullSize[0]
      ) {
        if (viewerLocked) {
          viewerLocked = false;
        }
        ...
        // We only want right and middle click
        return;
      }
      // Gets mouse button key from event.button
      // -2 or -4 is hold or short middle click, -1 or -3 is hold or short right click
      const rightButtonKey =
        !options.rightShortClickAndHold && options.rightShortClick ? -3 : -1;
      const middleButtonKey =
        !options.middleShortClickAndHold && options.middleShortClick ? -4 : -2;
      let mouseButtonKey = [null, middleButtonKey, rightButtonKey, null, null][
        event.button
      ];
    ...
    function clearMouseButtonTimers(mouseButtonKey) {
      ...
      if (preventDefaultMouseAction("get", mouseButtonKey) == true) {
        setTimeout(() => {
          preventDefaultMouseAction(false, mouseButtonKey);
        }, 10);
      }
    }
    ...
    function documentMouseUp(event) {
      ...
      const rightButtonKey =
        (shortPressRight || !options.rightShortClickAndHold) &&
        options.rightShortClick
          ? -3
          : -1;
      const middleButtonKey =
        (shortPressMiddle || !options.middleShortClickAndHold) &&
        options.middleShortClick
          ? -4
          : -2;
      let mouseButtonKey = [null, middleButtonKey, rightButtonKey, null, null][
        event.button
      ...
      switch (mouseButtonKey) {
        ...
        default:
          if (
            (mouseButtonKey == -3 && shortPressRight) ||
            (mouseButtonKey == -4 && shortPressMiddle)
          ) {
    ...
    function loadFullSizeImage() {
      ...
      if (!imgFullSize) {
        ...
        if (srcDetails.video) {
          ...
          var src = (
            srcDetails.audioUrl ? srcDetails.audioUrl : srcDetails.url
          ).replace(".audio", "");

          // audio controls are displayed on top of an image provided by extension: 'images/spectrogram.png'
          srcDetails.url = browser.runtime.getURL("images/spectrogram.png");
          srcDetails.audioUrl = src;

          imgFullSize = $('<img style="border: none" />')
            .appendTo(hz.hzViewer)
            .attr("src", srcDetails.url)
            .addClass("hzPlaceholder");
          ...
          hls.on("hlsError", function (event, data) {
            ...
            if (data.response) {
              ...
              if (
                data.response.data &&
                data.response.data.indexOf("MPD") != -1
              ) {
                ...
                var masterAddress = createMasterPlaylist(
                  MpdInfo,
                  createMediaPlaylist(MpdInfo, url),
                );
          ...
          function createMediaPlaylist(MpdInfo, url) {
            ...
            while (idx != -1) {
              ...
              var mapInit =
                ...
                item[key.indexOf("initialization", idx)] +
                '"\n';

              // #EXT-X-MLB-INFO:max-bw=999120,duration=4.000
              // totalDuration
              var rawtotalDuration =
                item[key.indexOf("mediaPresentationDuration")];
              var totalTime = timePatternFull.exec(rawDuration);
              if (totalTime) {
                var totalDuration =
                  parseFloat(totalTime[1]) * 3600 +
                  parseFloat(totalTime[2]) * 60 +
                  parseFloat(totalTime[3]);
              } else {
              ...
              var info =
                "#EXT-X-MLB-INFO:" +
                "max-bw=" +
                item[key.indexOf("bandwidth", idx)] +
                ",duration=" +
                totalDuration +
                "\n";
              ...
              var segmentDuration =
                parseFloat(
                  item[key.indexOf("duration", idx)] ||
                    item[key.indexOf("FBAverageDuration", idx)] ||
                    2000,
                ) / parseFloat(item[key.indexOf("timescale", idx)]); // Facebook uses "FBAverageDuration" instead of "duration"
              ...
              if (videoKey.test(segmentType)) {
                ...
                var video_info =
                  ...
                  'codecs="' +
                  item[key.indexOf("codecs", idx)] +
                  '",' +
                  'width="' +
                  item[key.indexOf("width", idx)] +
                  '",' +
                  'height="' +
                  item[key.indexOf("height", idx)] +
                  '",' +
                  'sar="' +
                  item[key.indexOf("sar", idx)] +
                  '",' +
                  "frame-duration=" +
                  item[key.indexOf("timescale", idx)] +
                  "\n";
                ...
                var audio_info =
                  ...
                  '",' +
                  'audioSamplingRate="' +
                  item[key.indexOf("audioSamplingRate", idx)] +
                  '"\n';

                // #EXT-X-MLB-AUDIO-CHANNEL-INFO:schemeIdUri="urn:mpeg:dash:23003:3:audio_channel_configuration:2011",value="2"
                var channel_info =
                  '#EXT-X-MLB-AUDIO-CHANNEL-INFO:schemeIdUri="' +
                  item[key.indexOf("schemeIdUri", idx)] +
                  '",' +
                  'value="' +
                  item[key.indexOf("value", idx)] +
                  "\n";
          ...
          function createMasterPlaylist(MpdInfo, mediaPlaylistBlob) {
            ...
            while (idx != -1) {
              ...
              if (videoKey.test(segmentType)) {
                ...
                videoMasterInfo +=
                  'CODECS="' + item[key.indexOf("codecs", idx)] + '",';
                videoMasterInfo +=
                  "RESOLUTION=" +
                  item[key.indexOf("width", idx)] +
                  "*" +
                ...
                if (key.indexOf("frameRate", idx) !== -1) {
                  videoMasterInfo +=
                    "FRAME_RATE=" + item[key.indexOf("frameRate", idx)] + ",";
                }
                videoMasterInfo +=
                  "BANDWIDTH=" + item[key.indexOf("bandwidth", idx)];
          ...
          hz.hzViewer.hzContainer = $('<div id="hzContainer"/>')
            .css(hz.hzContainerCss)
            .appendTo(hz.hzViewer);
          imgFullSize = $('<img style="border: none" />')
            .appendTo(hz.hzViewer.hzContainer)
            .on("load", srcFullSizeOnLoad)
        ...
        const urlDetails =
          srcDetails.audio && !srcDetails.video
            ? srcDetails.audioUrl
            : srcDetails.url;
    ...
    function addTimestampTrack(video) {
      ...
      for (var time = duration; time >= 0; time--) {
        ...
        finalTime +=
          ((hours > 0 || minutes > 0) && seconds < 10 ? "0" : "") + seconds;
        timer.push(finalTime);
      }
      for (var i = 0; i <= duration; i++) {
        track.addCue(
          new VTTCue(i, i == duration ? video.duration : i + 1, timer[i]),
        );
    ...
    function displayMsg(msgId) {
      ...
      $("<p/>")
        .text(browser.i18n.getMessage(msgId))
        .addClass("blinkWarning")
        .css(msgTextCss)
        .appendTo(msgDiv);
    ...
    function displayFullSizeImage() {
      ...
      if (!options.viewerShadowEnabled) imgFullSizeCss.boxShadow = "none";
      ...
      if (options.ambilightEnabled) {
        ...
        var background = $("<div/>");
        $(background)
          .css("width", 20 * screen.availWidth) // background canvas must be very large in case of zooming
      ...
      if (hz.currentLink) {
        ...
        if (loading && lowResSrc.indexOf("noimage") === -1) {
          var ext = srcDetails.url
            .substring(srcDetails.url.length - 3)
            .toLowerCase();
          if (ext != "gif" && ext != "svg" && ext != "png") {
      ...
      if (hz.currentLink) {
        ...
        if (
          ...
          var info =
            linkData.hoverZoomGalleryIndex +
            1 +
            "/" +
            linkData.hoverZoomGallerySrc.length;
          hzGallery = $("<div/>", { id: "hzGallery", text: info })
            .css(hzGalleryInfoCss)
            .prependTo(hz.hzViewer.hzContainer);
          if (
      ...
      if (!skipFadeIn && !hideKeyDown) {
        hz.hzViewer
          .hide()
          .fadeTo(options.fadeDuration, options.picturesOpacity);
      }
    ...
    function displayCaptionMiscellaneousDetails() {
      $("#hzAbove").remove();
      $("#hzBelow").remove();
      hzAbove = $("<div/>", { id: "hzAbove" })
        .css(hzAboveCss)
        .prependTo(hz.hzViewer);
      hzBelow = $("<div/>", { id: "hzBelow" })
        .css(hzBelowCss)
        .appendTo(hz.hzViewer);
    ...
    function displayCaptionMiscellaneous() {
      ...
      if (!options.captionDetailShadowEnabled) {
        //Toggles caption and detail box shadow off if option is disabled
      ...
      if (caption || miscellaneous) {
        if (options.captionLocation === "above")
          if (hzAbove.find("#hzCaptionMiscellaneous").length == 0)
            hzCaptionMiscellaneous = $("<div/>", {
              id: "hzCaptionMiscellaneous",
            })
              .css(hzCaptionMiscellaneousCss)
              .appendTo(hzAbove);
        ...
        if (caption)
          if (hzCaptionMiscellaneous.find("#hzCaption").length == 0)
            $("<div/>", { id: "hzCaption", text: caption })
              .css(hzCaptionCss)
              .appendTo(hzCaptionMiscellaneous);
        if (miscellaneous)
          if (hzCaptionMiscellaneous.find("#hzMiscellaneous").length == 0)
            $("<div/>", { id: "hzMiscellaneous", text: miscellaneous })
              .css(hzMiscellaneousCss)
              .appendTo(hzCaptionMiscellaneous);
      }
    }
        ...
        if (options.showDetailFilename) {
          if (hzDetails.find("#hzDetailFilename").length == 0)
            $("<div/>", {
              ...
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailFilename").text(details.filename);
        }
        if (options.showDetailHost) {
          if (hzDetails.find("#hzDetailHost").length == 0)
            $("<div/>", {
              ...
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailHost").text(details.host);
        }
        if (options.showDetailLastModified) {
          if (hzDetails.find("#hzDetailLastModified").length == 0)
            $("<div/>", {
              id: "hzDetailLastModified",
              text: details.lastModified,
              class: "hzDetail",
            })
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailLastModified").text(details.lastModified);
        }
        if (options.showDetailExtension) {
          if (hzDetails.find("#hzDetailExtension").length == 0)
            $("<div/>", {
              id: "hzDetailExtension",
              text: details.extension.toUpperCase(),
              class: "hzDetail",
            })
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailExtension").text(details.extension.toUpperCase());
        }
        if (options.showDetailContentLength) {
          if (hzDetails.find("#hzDetailContentLength").length == 0)
            $("<div/>", {
              id: "hzDetailContentLength",
              text: details.contentLength,
              class: "hzDetail",
            })
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailContentLength").text(details.contentLength);
        }
        if (options.showDetailDuration) {
          if (hzDetails.find("#hzDetailDuration").length == 0)
            $("<div/>", {
              ...
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailDuration").text(details.duration);
        }
        if (options.showDetailScale) {
          if (hzDetails.find("#hzDetailScale").length == 0)
            $("<div/>", {
              ...
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailScale").text(details.scale);
        }
        if (options.showDetailRatio) {
          if (hzDetails.find("#hzDetailRatio").length == 0)
            $("<div/>", {
              ...
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailRatio").text(details.ratio);
        }
        if (options.showDetailDimensions) {
          if (hzDetails.find("#hzDetailDimensions").length == 0)
            $("<div/>", {
              id: "hzDetailDimensions",
              text: details.dimensions,
              class: "hzDetail",
            })
              .css(hzDetailCss)
              .prependTo(hzDetails);
          else $("#hzDetailDimensions").text(details.dimensions);
        }
      }
    }
    ...
    function srcFullSizeOnError(e) {
      if (
        srcDetails.url === $(this).prop("src") ||
        srcDetails.url === unescape($(this).prop("src"))
      ) {
        let hoverZoomSrcIndex = hz.currentLink
          ? hz.currentLink.data().hoverZoomSrcIndex || 0
          : 0;
        ...
        if (hz.currentLink.data().abortOnFirstError) {
          console.info(
            "[HoverZoom] Failed to load source: " +
              srcDetails.url +
              "\nAborting.",
          );
          if (
            hz.currentLink.data().hoverZoomGallerySrc &&
            hz.currentLink.data().hoverZoomGallerySrc.length
          ) {
            hz.currentLink.data().hoverZoomGallerySrc = hz.currentLink
              .data()
              .hoverZoomGallerySrc.slice(
                0,
                hz.currentLink.data().hoverZoomGalleryIndex,
              );
            hz.currentLink.data().hoverZoomGalleryIndex = 0;
            hz.currentLink.data().hoverZoomSrc =
              hz.currentLink.data().hoverZoomGallerySrc[0];
          }
        } else if (
          hz.currentLink &&
          hoverZoomSrcIndex < hz.currentLink.data().hoverZoomSrc.length - 1
        ) {
          ...
          if (options.useSeparateTabOrWindowForUnloadableUrlsEnabled) {
            ...
            if (options.useSeparateTabOrWindowForUnloadableUrls == "window") {
              ...
              options.useSeparateTabOrWindowForUnloadableUrls == "tab"
            ) {
              openImageInTab(true); // do not focus tab
            }
          } else {
            console.warn(
              "[HoverZoom] Failed to load source: " + srcDetails.url,
            );
          }
        }
      }
    ...
    function getSrcDetails(link) {
      ...
      if (!srcDetails.audio || srcDetails.video) {
        if (srcDetails.naturalWidth)
          details.dimensions =
            srcDetails.naturalWidth + "x" + srcDetails.naturalHeight;
        if (srcDetails.naturalWidth)
          details.ratio = getImgRatio(
            srcDetails.naturalWidth,
            srcDetails.naturalHeight,
          );
        ...
        if (srcDetails.naturalWidth)
          details.scale =
            Math.round((100.0 * displayedWidth) / srcDetails.naturalWidth) +
            "%";
      }

      // if video comes with distinct url for audio then extension = video's extension
      details.extension = getExtensionFromUrl(
        srcDetails.audio && !srcDetails.video
          ? srcDetails.audioUrl
          : srcDetails.url,
        srcDetails.video,
    ...
    function getImgRatio(w, h) {
      ...
      let wRedux = redux[0];
      let hRedux = redux[1];
      if (wRedux == w || hRedux == h || wRedux > 20 || hRedux > 20)
    ...
    function imgLinksPrepared(links, name) {
      ...
      links.each(function () {
        var link = $(this),
          linkData = link.data();
        if (
          !linkData.hoverZoomSrc &&
          !linkData.hoverZoomGallerySrc &&
          !linkData.href &&
          $.isEmptyObject(linkData.meta)
        ) {
          ...
          if (linkData.hoverZoomGallerySrc) {
            if (!linkData.hoverZoomGalleryIndex)
              linkData.hoverZoomGalleryIndex = 0;
            linkData.hoverZoomGallerySrc = linkData.hoverZoomGallerySrc.map(
              function (srcs) {
          ...
          if (
            ...
            !options.ambilightEnabled &&
            !linkData.hoverZoomCaption
          ) {
    ...
    function prepareImgLinks() {
      ...
      for (var i = 0; i < hoverZoomPlugins.length; i++) {
        if (
          !options.disabledPlugins.includes(
            hoverZoomPlugins[i].name.replace(/[^\w\-_]/g, "").toLowerCase(),
          )
        )
    ...
    function prepareDownscaledImagesAsync(dontResetDelay) {
      ...
      prepareDownscaledImagesTimeout = setTimeout(
    ...
    function prepareDownscaledImages() {
      ...
      $("img")
        .filter(function () {
          ...
          if (
            this.src.toLowerCase().lastIndexOf(".jpg") !=
            this.src.length - 4
          ) {
            return false;
          }
        ...
        .one("mouseover.hoverZoom", function () {
          ...
          if (document.body.querySelector("#hzDownscaled") !== null) return;
          var hzDownscaled = $(
          ...
          hzDownscaled
            .on("load", function () {
              setTimeout(function () {
                ...
                if (
                  hzDownscaled.height() > heightAttr * 1.8 ||
                  hzDownscaled.width() > widthAttr * 1.8
                ) {
    ...
    function prepareImgLinksAsync(dontResetDelay) {
      ...
      prepareImgLinksTimeout = setTimeout(
        prepareImgLinks,
    ...
    function bindEvents() {
      ...
      wnd
        ...
        .scroll(cancelSourceLoading)
        .blur(cancelSourceLoading);
      ...
      $(document)
        ...
        .mouseleave(cancelSourceLoading);
    ...
    function bindJsaction() {
      $("[jsaction*=mousemove]").each(function () {
        ...
        if (
          ...
          $._data(this, "events")["mousemove"] == undefined
        )
      ...
      $("[jsaction*=keyup]").each(function () {
        if (
          $._data(this, "events") == undefined ||
          $._data(this, "events")["keyup"] == undefined
        )
    ...
    function documentOnMouseWheel(event) {
      ...
      if (
        ...
        let stepInit =
          ...
          (1.0 +
            Math.floor(
              Math.max(srcDetails.naturalWidth, srcDetails.naturalHeight) /
                1000.0,
            ));
        let step =
          zoomFactor < 2 ? stepInit : stepInit * Math.floor(zoomFactor);
        ...
        function (data) {
          ...
          if (createDataHeight > screen.availHeight) {
            ...
            let imgWidth = Math.round(
              (imgHeight * srcDetails.naturalWidth) / srcDetails.naturalHeight,
            );
            ...
            let imgHeight = Math.round(
              (imgWidth * srcDetails.naturalHeight) / srcDetails.naturalWidth,
            );
          ...
          let createDataTop = Math.round(
            screen.availHeight / 2 - createDataHeight / 2,
          );
          let createDataLeft = Math.round(
            screen.availWidth / 2 - createDataWidth / 2,
          );
      ...
      browser.runtime.sendMessage(
        ...
        function (data) {
          ...
          body[0].style.backgroundImage =
            "url(" + browser.runtime.getURL("images/spectrogram.png") + ")";
          ...
          setupScript.textContent =
            'document.querySelectorAll("[data-volume]").forEach(function(el){el.onloadeddata=function(){this.volume=parseFloat(this.dataset.volume)||0.5;};});';
          body.append(setupScript);

          let imgDim = hz.getImageDimensions(
            browser.runtime.getURL("images/spectrogram.png"),
          );
      ...
      browser.runtime.sendMessage(
        ...
        function (data) {
          ...
          if (createDataHeight > screen.availHeight) {
            ...
            let videoWidth = Math.round(
              (videoHeight * srcDetails.naturalWidth) /
                srcDetails.naturalHeight,
            );
            ...
            let videoHeight = Math.round(
              (videoWidth * srcDetails.naturalHeight) / srcDetails.naturalWidth,
            );
            video[0].height = videoHeight;
            video[0].width = videoWidth;
            createDataWidth = screen.availWidth;
    ...
    function openAudioInTab(background) {
      ...
      let setupScript = document.createElement("script");
      setupScript.textContent =
        'document.querySelector("audio").onloadeddata=function(){this.volume=parseFloat(this.dataset.volume)||0.5;};';
      body.append(setupScript);
    ...
    function openVideoInTab(background) {
      ...
      if (createDataHeight > screen.availHeight) {
        ...
        let videoWidth = Math.round(
          (videoHeight * srcDetails.naturalWidth) / srcDetails.naturalHeight,
        );
        video[0].height = videoHeight;
        video[0].width = videoWidth;
      } else if (createDataWidth > screen.availWidth) {
      ...
      if (contentLength) {
        if (!isNaN(contentLength) && contentLength > 0) {
          ...
          if (contentLength < 1000)
            infos.contentLength = contentLength.toFixed(0) + " KB";
          else infos.contentLength = (contentLength / 1024).toFixed(1) + " MB";
        }
      }
      let lastModified = headers["last-modified"];
      if (lastModified && lastModified.indexOf("01 Jan 1970") === -1)
        infos.lastModified = lastModified;
      return infos;
    }
    ...
    function downloadResource(url, filename, callback) {
      ...
      if (url.indexOf("//i.pximg.net/") !== -1) {
        browser.runtime.sendMessage({
          action: "downloadFileBlob",
          url: url,
          filename: filename,
          conflictAction: "uniquify",
        });
      } else {
        // all sites except pixiv.net
    ...
    function saveImage() {
      ...
      browser.runtime.sendMessage(
        { action: "getPermissionsContains", permissions: ["downloads"] },
    ...
    function copyImage() {
      ...
      setCanvasImage(url, (imgBlob) => {
        ...
        navigator.clipboard
          ...
          .then((e) => {
            cLog("Image copied to clipboard");
          })
          .catch((e) => {
            cLog(e);
          });
    ...
    function savePlaylistAsMP3MP4(filename) {
      ...
      if (fmp4Data["audio"].length) {
        const blobAudio = new Blob([arrayConcat(fmp4Data["audio"])], {
      ...
      if (fmp4Data["video"].length) {
        const blobVideo = new Blob([arrayConcat(fmp4Data["video"])], {
          type: "application/octet-stream",
        });
      ...
      if (fmp4Data["audiovideo"].length) {
        const blobAudioVideo = new Blob([arrayConcat(fmp4Data["audiovideo"])], {
          type: "application/octet-stream",
        });
    ...
    function getCaption() {
      let caption =
        hz.currentLink.data().hoverZoomCaption ||
        hz.currentLink.data().hoverZoomGalleryCaption ||
        "";
    ...
    function rotateGalleryImg(rot) {
      ...
      if (
        (data.hoverZoomGalleryIndex + rot < 0 ||
          data.hoverZoomGalleryIndex + rot >= len) &&
        !options.galleriesLoopEnabled
      ) {
      ...
      data.hoverZoomGalleryIndex =
        (data.hoverZoomGalleryIndex + rot + len) % len;
      updateImageFromGallery(link);
    ...
    function loadNextGalleryImage() {
      ...
      srcDetails.url =
        hz.currentLink.data().hoverZoomSrc[
          hz.currentLink.data().hoverZoomSrcIndex
        ];
        ...
        if (
          ...
          hzGallery.text(
            data.hoverZoomGalleryIndex +
              1 +
              "/" +
              data.hoverZoomGallerySrc.length,
          );
        }
    ...
    function updateImageFromGallery(link) {
      if (options.enableGalleries) {
        var data = link.data();
        data.hoverZoomSrc =
          data.hoverZoomGallerySrc[data.hoverZoomGalleryIndex];

        if (data.hoverZoomGalleryCaption) {
          data.hoverZoomCaption =
            data.hoverZoomGalleryCaption[data.hoverZoomGalleryIndex];
        } else {
          prepareImgCaption(link);
        }
      }
    ...
    function init() {
      ...
      body100pct =
        ...
        (body.css("padding-left") == "0px" &&
          ...
          (body.css("max-width") == "none" || body.css("max-width") == "100%"));
  ...
  getThumbUrl: function (el) {
    ...
    if (backgroundImage.indexOf("url") !== -1) {
      if (hoverZoom.isEmbeddedImg(backgroundImage))
        backgroundImageSrc = ""; // discard embedded images
      else
        backgroundImageSrc = backgroundImage
          .replace(/.*url\s*\(\s*(.*)\s*\).*/i, "$1")
          .replace(/"/g, "");
    }

    if (hoverZoom.isEmbeddedImg(el.src))
      imageSrc = ""; // discard embedded images
    else imageSrc = el.src;

    return (
      imageSrc || el.getAttribute("data-src") || backgroundImageSrc || el.href
    );
  },
  ...
  createHzViewer: function (displayNow) {
    if (!hoverZoom.hzViewer) {
      hoverZoom.hzViewer = $('<div id="hzViewer"></div>').appendTo(
        document.body,
      );
  ...
  displayImgLoader: function (status, position) {
    ...
    if (
      options.displayImageLoader === false &&
    ...
    if (hoverZoom.hzLoader == null) {
      ...
      if (!document.getElementById("hzLoaderStyle")) {
        ...
        style.textContent =
          "@keyframes hzSpin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }";
        document.head.appendChild(style);
      }
      hoverZoom.hzLoader = $(
        '<div id="hzLoader"><img src="' +
          browser.runtime.getURL("images/loading.svg") +
          '" style="opacity: 0.8; padding: 0; margin: 0; width: 32px; height: 32px; animation: hzSpin 1s linear infinite;" /></div>',
      );
    ...
    function preloadNextImage() {
      ...
      if (link.data().hoverZoomPreloaded) {
        ...
        var nextSrc = link.data().hoverZoomSrc
          ? link.data().hoverZoomSrc[hoverZoomSrcIndex]
          : "";
        if (nextSrc)
          $('<img src="' + nextSrc + '">')
            .on("load", function () {
  ...
  prepareFromDocument: function (link, url, getSrc, isAsync = false) {
    ...
    browser.runtime.sendMessage(
      { action: "ajaxRequest", url: url, method: "GET" },
      function (data) {
        let parser = new DOMParser();
        let doc = parser.parseFromString(data, "text/html");
        const httpRefresh = doc.querySelector(
          'meta[http-equiv="refresh"][content]',
        );
        if (httpRefresh) {
          let redirUrl = httpRefresh.content.substr(
            httpRefresh.content.toLowerCase().indexOf("url=") + 4,
          );
          if (redirUrl) {
        ...
        const handleSrc = function (src) {
          if (src) hoverZoom.prepareLink(link, src);
        };
  ...
  getFullUrl: function (url) {
    ...
    if (
      ...
      url.indexOf("https:") !== 0 &&
      url.indexOf("file:") !== 0 &&
      url.indexOf("blob:http:") !== 0 &&
      url.indexOf("blob:https:") !== 0
    ) {
      if (url.indexOf("//") !== 0) {
        if (url.indexOf("/") === 0) {
    ...
    if (currDeepLevel > maxDeepLevel) {
      ...
      for (var curr in jsonObj) {
        ...
        if (currDeepLevel == 1 && bShowInfo) {
          cLog('getKeysInJsonObject : Looking property "' + curr + '" ');
        }

        if (
          isRegex
            ? re.test(curr)
            : curr.toLowerCase() === searchKey.toLowerCase()
        ) {
          var r = {};
          r.key = curr;
          r.value = currElem;
        ...
        if (typeof currElem == "object") {
          ...
          // search again :D
          var deepKeys = hoverZoom.getKeysInJsonObject(
            currElem,
            searchKey,
            isRegex,
            maxDeepLevel,
  ...
  getValuesInJsonObject: function (
    jsonObj,
    searchValue,
    isRegex,
    isPartialMatch,
    ...
    if (currDeepLevel > maxDeepLevel) {
      ...
      for (var curr in jsonObj) {
        ...
        if (currDeepLevel == 1 && bShowInfo) {
          cLog('getValuesInJsonObject : Looking property "' + curr + '" ');
        }
        ...
        if (typeof currElem == "object") {
          // object is "object" and "array" is also in the eyes of "typeof"
          // search again :D
          var deepKeys = hoverZoom.getValuesInJsonObject(
            currElem,
            searchValue,
            isRegex,
            isPartialMatch,
          ...
          if (
            isRegex
              ? re.test(currElem)
              : isPartialMatch
                ? currElem.toString().toLowerCase().indexOf(searchValue) !== -1
                : currElem.toString().toLowerCase() === searchValue
          ) {
            var r = {};
            r.key = curr;
            r.value = currElem;
  ...
  decodeUnicode: function (strUnicode) {
    ...
    return strUnicode.replace(re, function (match, grp) {
  ...
  getBiggestSrcFromSrcset: function (srcset) {
    ...
    if (hoverZoom.isEmbeddedImg(srcset)) return undefined;
    ...
    if (srcset.indexOf(", ") != -1) {
      if (srcset.indexOf("x, ") != -1) {
        srcset = srcset.split("x, ");
      } else if (srcset.indexOf("w, ") != -1) {
