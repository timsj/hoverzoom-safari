      ...
      if (currDeepLevel > maxDeepLevel) {
        ...
        for (var curr in jsonObj) {
          ...
          if (currDeepLevel == 1 && bShowInfo) {
            cLog('getKeysInObject : Looking property "' + curr + '" ');
          }
      ...
      if (currDeepLevel > maxDeepLevel) {
        ...
        for (var curr in jsonObj) {
          ...
          if (typeof currElem == "object") {
            // object is "object" and "array" is also in the eyes of "typeof"
            // search again :D
            var deepKeys = getValuesInObject(
              currElem,
              searchValue,
              isRegex,
              maxDeepLevel,
    ...
    function extractData(link, dataJson, id) {
      ...
      try {
        ...
        if (
          ...
          meta.description = dataJson.description._content;
        ...
        if (id.indexOf("@") == -1) {
          if (!dataJson.secret_k && !dataJson.secret_h) {
            ...
            if (fullsizeUrl == undefined) {
              ...
              var requestUrl =
                "https://api.flickr.com/services/rest/?photo_id=" +
                id +
                "&method=flickr.photos.getSizes&format=json&nojsoncallback=1&api_key=9bb671af308f509d0c82146cbc936b3c";

              browser.runtime.sendMessage(
                { action: "ajaxGet", url: requestUrl },
                function (response) {
                  if (response == null) {
                    return;
                  }
...
var hoverZoomPluginFlickerA = {
  ...
  prepareImgLinks: function (callback) {
    ...
    $('a img[src*="static.flickr.com"], a img[src*="staticflickr.com"]')
      .filter(function () {
