...
var hoverZoom = {
    ...
    loadHoverZoom:function () {
        ...
        function displayMsg(msgId) {
            ...
            $('<p/>').text(browser.i18n.getMessage(msgId)).addClass('blinkWarning').css(msgTextCss).appendTo(msgDiv);
        ...
        function displayFullSizeImage() {
            ...
            browser.runtime.sendMessage({action:'getPermissionsContains', permissions: ['history']},
        ...
        function openAudioInWindow() {
            browser.runtime.sendMessage({action:'getItem', id:'popupBorder'}, function (data) {
                ...
                body[0].style.backgroundImage = 'url(' + browser.runtime.getURL('images/spectrogram.png') + ')';
        ...
        function downloadResource(url, filename, callback) {
            ...
            if (url.indexOf('//i.pximg.net/') !== -1) {
                browser.runtime.sendMessage({action: 'downloadFileBlob',
                                            url: url,
                                            filename: filename,
                                            conflictAction: 'uniquify'});
            } else { // all sites except pixiv.net
                // 1st attempt to download file (Chrome API)
                browser.runtime.sendMessage({action: 'downloadFile',
                                            url: url,
                                            filename: filename,
                                            conflictAction: 'uniquify'},
                                            function (downloadKO) {
        ...
        function banImage() {
            if (srcDetails.audioUrl) {
                browser.runtime.sendMessage({action:'banImage', url:srcDetails.audioUrl, location:window.location.href});
            } else if (srcDetails.url) {
                browser.runtime.sendMessage({action:'banImage', url:srcDetails.url, location:window.location.href});
            }
        }

        function saveImage() {
            cLog('checking for downloads permission');
            browser.runtime.sendMessage({action:'getPermissionsContains', permissions: ['downloads']}, 
                function(hasPermission) {
