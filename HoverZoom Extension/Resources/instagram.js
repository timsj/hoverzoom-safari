...
hoverZoomPlugins.push({
  ...
  prepareImgLinks: function (callback) {
      ...
      browser.runtime.sendMessage(
        {
          action: "ajaxGet",
          url:
            'https://www.instagram.com/graphql/query/?query_hash=d4d88dc1500312af6f937f7b804c68c3&variables={"user_id":"' +
            userId +
            '","include_chaining":true,"include_reel":false,"include_suggested_users":false,"include_logged_out_extras":false,"include_highlight_reels":true,"include_live_status":false}',
          headers: [{ header: "X-IG-App-ID", value: "936619743392459" }],
        },
        function (response) {
          var instagramUsersReelsHighlights =
    ...
    // header reels
    $('div[role="button"] img:not(div[role="presentation"] img)')
    ...
    $("a[href]")
      .filter(function () {
        return !/(\/reel\/|\/p\/)/.test($(this).prop("href"));
      })
      .on("mouseover", function () {
    ...
    function displayMedia(link, items0, callback) {
      ...
      if (videos) {
        ...
        link.data().hoverZoomCaption = items0.caption
          ? items0.caption.text
          : items0.accessibility_caption
            ? items0.accessibility_caption
            : items0.user.full_name;
        if (callback) callback(videoAudioSubtitlesUrl);
        else link.data().hoverZoomSrc = [videoAudioSubtitlesUrl];
      } else if (carousel) {
        let gallery = [];
        let captions = [];
        const caption = items0.caption
          ? items0.caption.text
          : items0.accessibility_caption
            ? items0.accessibility_caption
            : items0.user.full_name;
...
hoverZoomPlugins.push({
  ...
  prepareImgLinks: function (callback) {
    ...
    $('a[href*="lookaside.instagram.com"]:not(.hoverZoomMouseover)')
